...
function recommendGames($unratedGames, $genres, $user_id, $limit = 8) {
    $recommendations = [];
...
echo "<ol>";
$_SESSION['recommandations']=array();
foreach ($recommendations as $game) {
    echo "<li>" . htmlspecialchars($game["title"]) . " (ID: " . $game["id"] . ")</li>";
    //crée une varaible de session pour stocker les recommandations d'un user dès qu'il est connecté et les afficher dans la page Index0
    $_SESSION['recommandations'][]=array($game["id"],$game["title"]);
